...
3. **Find the flipping test.** `replay <file> -v` prints per frame:
   `pose=… I1 M1 R0 L0 curl=2 tips=1.12 T1(s1 c1 u0) ext=0.31 palm=0.16 span=0.45 up=0.99 hold=5/15 speed=0.03`. Read the
   ...
   as a fraction of the frame, span = knuckle width in palm lengths, up = middle knuckle above the
   wrist in palm lengths; `SMALL` or `ANGLED` after it names the gate that refused the pose). The flag that
...
- `replay <file> --csv > file.csv` — one row per frame of what the current pipeline derives (t,
  hand, pose, gated, speed, palm, extent, span, tips, upright, hold, candidate, fired). Use it for any measurement
